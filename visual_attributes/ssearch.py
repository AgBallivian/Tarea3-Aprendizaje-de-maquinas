...
sys.path.append("H:/Universidad/Semestre 9/Aprendizaje de maquinas/Tarea 3/convnet2")
...
if __name__ == '__main__' :
    ...
    if pargs.mode == 'search' :
        #Cargar los features
        features, n = ssearch.load_features()
        color_data = features[:, :n]
        # texture_data = features[:, n:]  
        ...
        catalog_file = "dataset_ssearch/colors/ssearch/catalog.txt"  # Ruta al archivo catalog.txt
        folder_index = 1  # Índice de la carpeta de nivel superior en la ruta (contando desde 0)
        ...
        with open(catalog_file, 'r') as f:
            for line in f:
                ...
                folder_name = image_path[folder_index]
                labels.append(folder_name)

        # Verificar las etiquetas obtenidas
        print(labels)

        print("Creando UMAP")    
        # Obtener los embeddings para el conjunto de datos de color
        embeddings_color = umap.UMAP().fit_transform(color_data)

        # Obtener los embeddings para el conjunto de datos de textura
        # embeddings_texture = umap.UMAP().fit_transform(texture_data)
            
        # Grafico para el bloque de color
        plt.scatter(embeddings_color[:, 0], embeddings_color[:, 1])
        plt.title('Visualización UMAP - Bloque de Color')
        plt.xlabel('Componente 1')
        plt.ylabel('Componente 2')
        plt.show()

        # # Grafico para el bloque de textura
        # plt.scatter(embeddings_texture[:, 0], embeddings_texture[:, 1])
        # plt.title('Visualización UMAP - Bloque de Textura')
        # plt.xlabel('Componente 1')
        # plt.ylabel('Componente 2')
        # plt.show() 
